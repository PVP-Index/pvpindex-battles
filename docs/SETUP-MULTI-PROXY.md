...
Each proxy connects to a shared Redis instance and communicates via Pub/Sub. Player locations, battle states, and challenges are all synchronized across the network.
...
3. **Player Tracking**: Player join/leave/switch events are broadcast via Redis so every proxy knows every player's location.
4. **Cross-Proxy Challenges**: When a player challenges someone on another proxy, the challenge is routed through Redis to the target proxy, which forwards it to the target player's backend.
5. **Battle Events**: Battle start/end events are broadcast so all proxies can report accurate global battle counts.
...
|---------|-------------|
| "Failed to initialize network layer" | Redis is unreachable or password is wrong |
